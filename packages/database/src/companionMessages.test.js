...
describe('companion messages', () => {
    ...
    it('prunes messages older than retention days', () => {
        ...
        rawDb
            .prepare(`INSERT INTO companion_messages (id, character_id, role, content, source, status, created_at)
         VALUES ('old_msg', 'ann', 'user', 'Old message', 'panel', 'ok', ?)`)
            .run(oldDate);
        // Before prune: 2 rows
        expect(rawDb.prepare('SELECT COUNT(*) as cnt FROM companion_messages').get().cnt).toBe(2);
