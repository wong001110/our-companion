...
export function assessAttention(input: AssessAttentionInput): AttentionAssessment {
  ...
  const deservesAttention =
    attentionValue >= attentionCost + 20 &&
...
function baseDecision(action: CompanionDecision['action'], input: {
  timing: CompanionDecision['timing'];
  priority: CompanionDecision['priority'];
  reason: string;
}): CompanionDecision {
  return {
