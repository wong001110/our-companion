...
function makeStep(toolName, args) {
    return {
        id: createId('step'),
        toolName,
        args,
 ...
 */
export function planActionFromRules(text) {
...
export async function planActionFromLlm(text, deps) {
    try {
        const raw = await deps.completeJson([
            {
                role: 'system',
                content: 'You are Ann, a desktop companion. Convert the user request into a JSON action plan. ' +
        ...
        if (!parsed || parsed.steps.length === 0 || parsed.steps[0].tool_name === 'none')
            return undefined;
        return {
            id: createId('plan'),
            ...
            steps: parsed.steps.map((s) => ({
                ...
                args: s.args,
                requiredScopes: (s.required_scopes ?? scopesForTool(s.tool_name)),
            })),
            source: 'llm',
            createdAt: nowIso(),
        };
...
export async function runActionPlan(plan, deps, correlationId) {
