...
export function decayEmotion(emotion, date = new Date()) {
...
export function applyEmotionEvent(emotion, event) {
    const next = { ...emotion };
...
export function selectIntent(state, context) {
    if (context.userCommand || (context.pendingTasks ?? 0) > 0)
        return 'helping_task';
    if ((context.availableDiscoveries?.length ?? 0) > 0)
        return 'sharing_discovery';
    if (context.recentMemoryActivity)
        return 'reviewing_memory';
    if (context.reflectionDue)
        return 'reflecting_journey';
    if (!context.userActive && dominantEmotion(state.emotion) === 'tired')
        return 'waiting';
    if (!context.userActive && state.emotion.curious > 50)
        return 'wandering';
    return 'organizing_backpack';
}
export function transitionState(current, intent, emotion) {
    ...
    if (intent === 'reviewing_memory' || intent === 'reflecting_journey')
        return 'thinking';
    if (intent === 'organizing_backpack')
        return current === 'organizing_backpack' ? 'idle' : 'organizing_backpack';
    if (emotion === 'tired')
        return current === 'sleeping' ? 'idle' : 'sleeping';
    if (current === 'idle')
        return 'walking';
    if (current === 'walking')
        return 'observing';
...
export function animationFor(intent, state, emotion, availableAnimations) {
    const variants = [];
    if (intent === 'wandering' && emotion === 'tired')
        variants.push('walk_tired', 'sleep');
    if (intent === 'wandering' && emotion === 'excited')
        variants.push('walk_excited');
    if (intent === 'sharing_discovery' && emotion === 'shy')
        variants.push('discover_shy');
    if (intent === 'sharing_discovery' && emotion === 'excited')
        variants.push('discover_excited');
    if (state === 'talking' && emotion === 'happy')
        variants.push('talk_happy');
    if (state === 'talking' && emotion === 'confused')
        variants.push('talk_confused');
    if (state === 'executing' && emotion === 'focused')
        variants.push('task_start');
    if (state === 'returning' && emotion === 'proud')
        variants.push('task_success');
    if (state === 'returning' && emotion === 'confused')
        variants.push('task_fail');
    const baseByState = {
        idle: 'idle',
        walking: 'walk',
        sleeping: 'sleep',
...
export function advanceCharacter(state, context) {
