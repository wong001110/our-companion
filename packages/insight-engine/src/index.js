...
export function scoreInsight(input) {
    const finalScore = clamp01(input.confidence * 0.2 +
        input.novelty * 0.2 +
...
export function narrateInsight(insight) {
    return [
...
export function generateInsights(input) {
    ...
    const primaryCandidate = [...candidates].sort((left, right) => right.relevanceScore + right.noveltyScore + right.usefulnessScore - (left.relevanceScore + left.noveltyScore + left.usefulnessScore))[0];
    const relatedPattern = input.patterns.find((pattern) => input.curiosityTarget.relatedPatternIds?.includes(pattern.id)) ?? input.patterns[0];
    ...
    const insight = {
        ...
        whyItMatters: input.curiosityTarget.explorationType === 'challenge'
        ...
        suggestedAction: input.curiosityTarget.explorationType === 'practical'
...
export function selectPrimaryInsight(insights) {
...
export function generateCognitiveInsight(input) {
    const pattern = [...input.patterns].sort((left, right) => right.strength - left.strength)[0];
    const concept = input.concepts[0];
    const candidate = [...input.discoveryCandidates].sort((left, right) => right.relevanceScore + right.usefulnessScore - (left.relevanceScore + left.usefulnessScore))[0];
    if (!pattern && !concept && !candidate)
        return undefined;
    const title = pattern?.title ?? concept?.name ?? candidate?.title ?? 'New cognitive insight';
    const explanation = pattern?.description ??
        pattern?.summary ??
        concept?.summary ??
        candidate?.summary ??
    ...
    const growthValue = Math.round(Math.min(100, ((pattern?.strength ?? 0.55) * 45 + (candidate?.usefulnessScore ?? 0.55) * 35 + confidence * 20)));
    return {
        id: createId('insight'),
        title,
        explanation,
