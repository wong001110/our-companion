...
export function createCuriosityBudget(date: string, total = 100): CuriosityBudget {
  return {
    date,
    total,
    used: 0,
...
export function spendCuriosityBudget(budget: CuriosityBudget, cost: number): CuriosityBudget {
...
export function calculateCuriosityMomentum(input: CuriosityMomentumInput): number {
  return clamp01(0.5 + input.savedCount * 0.12 + (input.followUpCount ?? 0) * 0.1 - input.ignoredCount * 0.16);
}

export function assessCuriosity(input: AssessCuriosityInput): CuriosityAssessment {
...
export function scoreCuriosityInvestment(input: Omit<CuriosityInvestment, 'id' | 'score' | 'updatedAt'>): CuriosityInvestment {
  const score = clamp01(
    input.longTermValue * 0.24 +
...
function rejectedTopics(feedback: DiscoveryFeedback[] = []): Set<string> {
  return new Set(
    feedback
...
export function generateCuriosityTargets(input: GenerateCuriosityTargetsInput): CuriosityTarget[] {
  ...
  for (const pattern of strongPatterns.slice(0, 3)) {
    const mode: ExplorationType = pattern.type === 'contradiction' ? 'challenge' : 'deepening';
    targets.push(
      createTarget({
        userId: input.userId,
        companionId,
  ...
  for (const node of graphNodes.slice(0, 3)) {
    const explorationType: ExplorationType = node.type === 'technology' ? 'practical' : 'similar';
    targets.push(
      createTarget({
        userId: input.userId,
        companionId,
