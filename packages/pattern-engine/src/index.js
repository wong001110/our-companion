...
export function scorePattern(input) {
    const finalScore = clamp01(input.frequency * 0.35 + input.recency * 0.25 + input.emotionalWeight * 0.2 + input.feedbackWeight * 0.2);
...
export function detectPatterns(input) {
    const textItems = [
        ...
        ...input.discoveryHistory.map((discovery) => ({
            sourceType: discovery.status === 'rejected' ? 'dismissed_discovery' : 'saved_discovery',
...
export function detectCognitivePatterns(input) {
    const timestamp = nowIso();
    const patterns = [];
    ...
    for (const discovery of input.discoveries) {
        for (const tag of discovery.tags) {
            const sources = sourceCounts.get(tag) ?? new Set();
