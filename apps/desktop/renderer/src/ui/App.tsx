...
function SocialCard() {
  ...
  return <PaperCard title="Social" tape className="settings-panel">
    ...
    <h3>Visit Session</h3>{liveVisit ? <div className="online-user-info"><strong>{visitSessionMessage(liveVisit, userId)}</strong><p>Snapshot pack: {liveVisit.assetPackId}</p>{liveVisit.state === 'active' && <p>{visualVisitMessage(visualVisit, liveVisit, userId)}</p>}<div className="action-row">{liveVisit.state === 'preparing' && (currentUserReadyForVisit ? <span>Your side is prepared.</span> : <button disabled={busyAction} onClick={() => void action(() => window.ourCompanion.network.visits.sessions.prepare(liveVisit.id))}>{busyAction ? 'Preparing…' : 'Prepare'}</button>)}{liveVisit.state === 'ready' && liveVisit.hostUserId === userId && <button disabled={busyAction} onClick={() => void action(() => window.ourCompanion.network.visits.sessions.start(liveVisit.id))}>Start Visit</button>}<button disabled={busyAction} onClick={() => void action(() => window.ourCompanion.network.visits.sessions.end(liveVisit.id))}>{liveVisit.state === 'preparing' || liveVisit.state === 'ready' ? 'Cancel Visit' : 'End Visit'}</button></div></div> : latestTerminalVisit ? <div className="online-user-info"><strong>{visitSessionMessage(latestTerminalVisit, userId)}</strong>{latestTerminalVisit.endReason && <p>Reason: {latestTerminalVisit.endReason.replaceAll('_', ' ')}</p>}</div> : <p>No current Visit Session.</p>}</>}
