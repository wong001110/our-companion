...
function SocialCard() {
  ...
  const clearSocialState = useCallback(() => {
    setFriends([]); setIncoming([]); setOutgoing([]); setBlocked([]); setVisitIncoming([]); setVisitOutgoing([]); setVisitSessions([]); setCanSendVisit(false); setLookup(undefined); setFriendCompanion(undefined); setFriendAssetStatus(''); setError(''); setFriendCode('');
  }, []);
  const refresh = async () => {
    ...
    setLoading(true);
    try {
      const [nextFriends, nextIncoming, nextOutgoing, nextBlocked, presence, nextVisitIncoming, nextVisitOutgoing, nextVisitSessions, mine] = await Promise.all([
      ...
      const activeCompanion = mine.companions.find((companion) => companion.id === mine.activeNetworkCompanionId);
      const activePack = activeCompanion?.assetPacks.find((pack) => pack.id === activeCompanion.activeAssetPackId && pack.status === 'active');
      setIncoming(nextIncoming); setOutgoing(nextOutgoing); setBlocked(nextBlocked); setVisitIncoming(nextVisitIncoming); setVisitOutgoing(nextVisitOutgoing); setVisitSessions(nextVisitSessions); setCanSendVisit(Boolean(visitsAvailable && activeCompanion?.published && activeCompanion.visibility === 'friends_only' && activePack)); setError('');
  ...
  useEffect(() => {
    ...
    void refresh();
  }, [available, scope, status?.socialRevision, visitsAvailable]);

  if (!available) return <div data-testid="social-panel"><PaperCard title="Social" tape className="settings-panel"><p>{socialAvailabilityMessage(status)}</p></PaperCard></div>;
  const action = async (operation: () => Promise<unknown>, options: { clearLookup?: boolean } = {}) => { if (busyAction) return; setBusyAction(true); try { await operation(); await window.ourCompanion.network.presence.sendActivity(); if (options.clearLookup ?? true) setLookup(undefined); await refresh(); } catch (cause) { setError(messageForSocialError(cause)); } finally { setBusyAction(false); } };
  ...
  return <div data-testid="social-panel"><PaperCard title="Social" tape className="settings-panel">
    ...
    <h3>Visit Session</h3>{liveVisit ? <div data-testid="visit-session-state" data-state={liveVisit.state} className="online-user-info"><strong>{visitSessionMessage(liveVisit, userId)}</strong><p>Snapshot pack: {liveVisit.assetPackId}</p>{liveVisit.state === 'active' && <p>{visualVisitMessage(visualVisit, liveVisit, userId)}</p>}<div className="action-row">{liveVisit.state === 'preparing' && (currentUserReadyForVisit ? <span>Your side is prepared.</span> : <button data-testid="prepare-visit" disabled={busyAction} onClick={() => void action(() => window.ourCompanion.network.visits.sessions.prepare(liveVisit.id))}>{busyAction ? 'Preparing…' : 'Prepare'}</button>)}{liveVisit.state === 'ready' && liveVisit.hostUserId === userId && <button data-testid="start-visit" disabled={busyAction} onClick={() => void action(() => window.ourCompanion.network.visits.sessions.start(liveVisit.id))}>Start Visit</button>}<button data-testid="end-visit" disabled={busyAction} onClick={() => void action(() => window.ourCompanion.network.visits.sessions.end(liveVisit.id))}>{liveVisit.state === 'preparing' || liveVisit.state === 'ready' ? 'Cancel Visit' : 'End Visit'}</button></div></div> : latestTerminalVisit ? <div data-testid="visit-session-state" data-state={latestTerminalVisit.state} className="online-user-info"><strong>{visitSessionMessage(latestTerminalVisit, userId)}</strong>{latestTerminalVisit.endReason && <p>Reason: {latestTerminalVisit.endReason.replaceAll('_', ' ')}</p>}</div> : <p data-testid="visit-session-state">No current Visit Session.</p>}</>}
...
function visitSessionMessage(session: VisitSessionSummary, userId: string): string {
  ...
  if (session.state === 'active') return 'Visit active.';
...
function visualVisitMessage(visual: import('@our-companion/shared').VisualVisitRendererState, session: VisitSessionSummary, userId: string): string {
  ...
  return 'Preparing Visitor assets…';
}
