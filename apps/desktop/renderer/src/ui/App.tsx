...
function SocialCard() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setIncoming(nextIncoming); setOutgoing(nextOutgoing); setBlocked(nextBlocked); setVisitIncoming(nextVisitIncoming); setVisitOutgoing(nextVisitOutgoing); setVisitSessions(nextVisitSessions); setCanSendVisit(Boolean(activeCompanion?.published && activeCompanion.visibility === 'friends_only' && activePack)); setError('');
