...
export function JourneysPage({ journeys, timeline, onRefresh }: { journeys: CompanionJourney[]; timeline: JourneyMilestoneV2[]; onRefresh: () => Promise<void> }) {
  ...
  return (
    <NotebookPage eyebrow={t(lang, 'journey_eyebrow')} title={t(lang, 'journey_title')} note={t(lang, 'journey_note')} marker="journey">
      ...
      <div className="journey-list">
        {journeys.map((journey, index) => <PaperCard key={journey.id} className="journey-card" tape><div className="journey-main"><div><h3>{journey.title}</h3><p>{journey.description ?? t(lang, 'journey_default_desc')}</p><p className="soft-pill">{timeline[index] ? t(lang, 'journey_progress_with_next') : t(lang, 'journey_progress_without_next')}</p></div></div><StickyNote title={t(lang, 'journey_next_step')} compact><p>{timeline[index]?.description ?? timeline[index]?.title ?? t(lang, 'journey_default_next_step')}</p></StickyNote></PaperCard>)}
