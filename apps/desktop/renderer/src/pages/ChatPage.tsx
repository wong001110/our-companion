...
export function ChatPage() {
  ...
  async function clearHistory() {
    setClearing(true);
    try { await window.ourCompanion.companion.clearHistory(); setMessages([]); } finally { setClearing(false); setConfirmClear(false); }
  }
    ...
    <NotebookPage eyebrow={t(lang, 'chat_eyebrow')} title={t(lang, 'chat_title')} note={t(lang, 'chat_note')}>
      <section className="chat-paper chat-view">
        ...
        <div className="chat-composer">
          ...
          <div className="action-row"><button onClick={() => void sendMessage()} disabled={sending || !input.trim()}>{sending ? t(lang, 'chat_sending') : t(lang, 'chat_send')}</button>{confirmClear ? <><span>{t(lang, 'chat_clear_confirm')}</span><button onClick={() => void clearHistory()} disabled={clearing}>{t(lang, 'chat_clear_yes')}</button><button onClick={() => setConfirmClear(false)}>{t(lang, 'chat_clear_cancel')}</button></> : <button className="btn-ghost" onClick={() => setConfirmClear(true)}>{t(lang, 'chat_clear')}</button>}</div>
