...
export function VisitInvitationRow({ lang, invitation, direction, username, disabled, disabledReason, onAccept, onDecline, onCancel }: {
  ...
  return <OperationalRow
    ...
    supporting={<>{invitation.companionDescription && <p>{invitation.companionDescription}</p>}<p>{invitation.companionTags.join(' · ') || t(lang, 'social_no_public_tags')}</p>{invitation.topic && <div className="visit-invitation-topic-preview"><strong>{lang === 'zh-CN' ? '主题预览：' : 'Topic preview: '}{invitation.topic.title}</strong><p>{invitation.topic.summary}</p>{invitation.topic.tags.length > 0 && <small>{invitation.topic.tags.join(' · ')}</small>}</div>}<p>{t(lang, direction === 'incoming' ? 'social_visit_role_host' : 'social_visit_role_visitor')} · {t(lang, 'social_visit_visual_only')}</p><p>{t(lang, 'social_expires', { time: '' })} <time dateTime={invitation.expiresAt}>{formatDateTime(invitation.expiresAt, lang)}</time></p></>}
