...
function withAssets(run: (root: string) => void) {
  ...
  for (const name of ['Idle_Neutral', 'Enter', 'Leave']) fs.writeFileSync(path.join(assets, `${name}.png`), createPngFixture(300, 300));
...
describe('S3 asset manifest builder', () => {
  ...
  it('rejects truncated or CRC-corrupt PNG animation files', () => {
    for (const invalid of [
      createPngFixture(300, 300).subarray(0, -8),
      (() => {
        const bytes = Buffer.from(createPngFixture(300, 300));
        bytes[bytes.length - 5] ^= 0xff;
        return bytes;
      })(),
    ]) {
      withAssets(root => {
        fs.writeFileSync(path.join(root, 'companions', 'companion-1', 'assets', 'animations', 'Enter.png'), invalid);
        expect(() => buildAssetManifest({ companionId: 'companion-1', pathOptions: options(root) }))
          .toThrow('ASSET_PACK_FILE_INVALID');
      });
    }
  });
  it('uses the shared Walk timing in portable metadata', () => withAssets(root => {
    const animations = path.join(root, 'companions', 'companion-1', 'assets', 'animations');
    fs.writeFileSync(path.join(animations, 'Walk_Right.png'), createPngFixture(600, 300, 0, 1));
