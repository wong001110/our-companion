...
const api: OurCompanionApi = {
  ...
  discovery: {
    ...
    listChannels: () => invoke('discovery:listChannels'),
    updateChannelState: (input) => invoke('discovery:updateChannelState', input),
    exploreChannelNow: (platformId) => invoke('discovery:exploreChannelNow', platformId),
    ...
    getDiscoveryProfile: () => invoke('discovery:getDiscoveryProfile'),
...
if (process.env.OUR_COMPANION_SMOKE_TEST === '1') {
  api.smoke = {
    ...
    presentDiscoveryFixture: (input) => invoke('smoke:presentDiscoveryFixture', input),
